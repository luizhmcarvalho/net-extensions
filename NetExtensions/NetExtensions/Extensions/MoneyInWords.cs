namespace System
{
    ...
    {
        private static readonly string[] Currency = {"real", "reais", "centavo", "centavos"};

        private static readonly string[] Unidade =
            {"", "um", "dois", "três", "quatro", "cinco", "seis", "sete", "oito", "nove"};

        private static readonly string[] Dezena =
            {"", "dez", "vinte", "trinta", "quarenta", "cinquenta", "sessenta", "setenta", "oitenta", "noventa"};

        private static readonly string[] Centena =
        {
            "", "cento", "duzentos", "trezentos", "quatrocentos", "quinhentos", "seiscentos", "setecentos",
            "oitocentos", "novecentos"
        };

        private static readonly string[] DezAux =
        ...
        public static string GetExtensoFmt(this decimal pValue, string pUnit, string pUnits, string pFrac,
            string pFracs)
        {
            ...
            if (result != "" && ExtBilhao != "")
                if (ValBilhao < 100 || ValBilhao % 100 == 0)
                    if (ExtMilhao + ExtMilhar + ExtCentena == "")
                        result += " e ";
                    else
                        result += ", ";
                else
            ...
            if (result != "")
                if (ExtMilhar + ExtCentena == "")
                    ...
                    if (ValMilhar < 100 || ValMilhar % 100 == 0)
                        result += " e ";
                    else
                        result += ", ";
            ...
            if (result != "" && ExtCentena != "")
                if (ValCentena < 100 || ValCentena % 100 == 0)
                    result += " e ";
                else
                    result += ", ";
