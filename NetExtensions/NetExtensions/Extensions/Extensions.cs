...
namespace System
{
	public static class ExtensionMethodsSys
	{
		...
		{
			var r = Parallel.ForEach(pLista, pAction);
		...
		{
			return pLista.AsParallel().Select(pAction).ToList();
		}
		...
		public static Dictionary<TKey, TValue> ToDictionary<TKey, TValue>(
			this IEnumerable<KeyValuePair<TKey, TValue>> pLista)
		{
		...
		public static FileInfo SaveToFile<T>(this IEnumerable<T> pList, string pFileName)
		{
			...
			var lArquivo = new StreamWriter(pFileName, false, Encoding.GetEncoding("iso-8859-15"));
			...
			Func<TOuter, TInner, TResult> resultSelector, IEqualityComparer<TKey> comparer)
		{
			...
			var innerJoinItems = comparer != null
				? inner.Where(innerItem =>
						!outerLookup.Select(x => x.Key).Contains(innerKeySelector(innerItem), comparer))
			...
			return outer
				.SelectMany(outerItem =>
				{
					var innerItems = comparer != null
						? innerLookup.Where(x => comparer.Equals(x.Key, outerKeySelector(outerItem)))
							.SelectMany(x => x.ToList())
						: innerLookup[outerKeySelector(outerItem)];

					return innerItems.Any() ? innerItems : new[] { default(TInner) };
				}, resultSelector)
				.Concat(innerJoinItems);
		}


		public static IEnumerable<TResult> Except<TResult>(this IEnumerable<TResult> pList, IEnumerable<TResult> pValue,
			Func<TResult, object> pKey)
		{
			return pList.Except(pValue, new GenericCompare<TResult>(pKey));
		}

		public static IEnumerable<TResult> Except<TResult>(this IEnumerable<TResult> pList, IEnumerable<TResult> pValue,
			Func<TResult, TResult, bool> pEquality)
		{
		...
		{
			var lResult = string.Join(pSeparator, pLista);
		...
		public class PivotTable<TRow, TColumn, TValue> : Dictionary<TRow, PivotRow<TColumn, TValue>>
		{
			...
			public TColumn[] Columns => _columns.ToArray();
			...
			{
				if (!ContainsKey(row)) Add(row, new PivotRow<TColumn, TValue>());
				if (!this[row].ContainsKey(column)) this[row].Add(column, value);
				else this[row][column] = value;
			}
		}

		public static PivotTable<TRow, TColumn, TValue> Pivot<T, TRow, TColumn, TValue>(this IEnumerable<T> pList,
			Func<T, TRow> pKey1, Func<T, TColumn> pKey2, Func<T, TValue> pValueSelector)
		{
			var result = new PivotTable<TRow, TColumn, TValue>();
		...
		public static PivotTable<TRow, TColumn, decimal> Pivot<T, TRow, TColumn>(this IEnumerable<T> pList,
			Func<T, TRow> pKey1, Func<T, TColumn> pKey2, Func<T, decimal> pValueSelector)
		{
			var result = new PivotTable<TRow, TColumn, decimal>();
			result._columns = pList.Select(pKey2).Distinct().ToArray();
			pList.GroupBy(x => new { row = pKey1(x), column = pKey2(x) })
		...
		{
			var c = 0;
			for (var i = pLista.Count - 1; i >= 0; i--)
		...
		/// <summary>
		///     Replaces an item that matches the conditions of the specified predicate from the System.Collections.Generic.List
		///     &lt;T&gt;.
		/// </summary>
		/// <typeparam name="T">object type</typeparam>
		...
		public static string ConvertToQueryLinq<T>(this string filter) where T : class
		{
			try
			{
				if (filter.IsNullOrEmpty()) throw new Exception("Filter not informed");

				var queryResult = "";
				var propertyName = filter.Split(":")[0];
				var propertyValue = filter.Split(":")[1];

				var propertyType = typeof(T).GetProperties().FirstOrDefault(x => x.Name.ToLower() == propertyName.ToLower())?.PropertyType;
				if (propertyType == null) throw new Exception("Property not found");

				if (propertyType == typeof(string))
				{
					queryResult = $"{propertyName}.Contains(\"{propertyValue}\")";
				}
				else
				{
					queryResult = $"{propertyName} = {propertyValue}";
				}
		...
		{
			var index = pString.LastIndexOf(oldValue);
			if (index >= 0)
			{
		...
		{
			return CultureInfo.CurrentCulture.CompareInfo.Compare(s1.RemoveCaracters(' ', '-'),
					   s2.RemoveCaracters(' ', '-'),
					   CompareOptions.IgnoreCase | CompareOptions.IgnoreKanaType | CompareOptions.IgnoreNonSpace) == 0
				? true
				: false;
		}
		...
		{
			return CultureInfo.CurrentCulture.CompareInfo.IndexOf(source, value, 0,
		...
		public static string PutMask(this string s, string pMask)
		{
			...
			var lMaskProvider = new MaskedTextProvider(pMask);
			lMaskProvider.Set(s);
		...
		/// <summary>
		///     Substitui o item de formatação em uma string especificada pela string representada pelo objeto correspondente no
		///     array
		/// </summary>
		...
		public static DateTime ToDateTime(this string pData, string pNomeCampoData)
		{
			...
			{
				throw new Exception("{0}: \'{1}\' inválida!".Formata(pNomeCampoData ?? "Data", pData));
			}
		...
		public static DateTime ToDateTime(this string pData)
		{
			...
			{
				throw new Exception("{0}: \'{1}\' inválida!".Formata("Data", pData));
			}
		...
		public static DateTime? ToNullableDateTime(this string pData, string pNomeCampoData = null)
		{
			...
			return pData.ToDateTime(pNomeCampoData);
		}
		...
		public static int ToInt32(this string pInt)
		{
			if (pInt.IsNullOrEmpty()) return 0;
			try
			{
				return int.Parse(pInt);
			}
			catch (FormatException ex)
			{
				throw new Exception(string.Format("\'{0}\' não é um inteiro válido.", pInt));
			}
		}
		...
		public static long ToInt64(this string pInt)
		{
			...
			try
			{
				return long.Parse(pInt);
			}
			catch (FormatException ex)
			{
				throw new Exception($"'{pInt}' não é um inteiro válido.");
			}
		}
		...
		public static decimal ToDecimal(this string pDecimal)
		{
			...
			try
			{
				return decimal.Parse(pDecimal);
			}
			catch (FormatException ex)
			{
				throw new Exception(string.Format("\'{0}\' não é um número válido.", pDecimal));
			}
		}
		...
		public static Guid ToGuid(this string pGuid)
		{
			...
			try
			{
				return Guid.Parse(pGuid);
			}
			catch (FormatException ex)
			{
				throw new Exception(string.Format("\'{0}\' não é um Guid válido.", pGuid));
			}
		}
		...
		public static string InverterData(this string pData)
		{
			...
			var lDataInvertida = "";
			for (var i = 0; i < lArrayString.Length; i++)
			{
		...
		public static int ToInt32(this bool pBool)
		{
		...
		{
			var lData = pDataReferencia ?? DateTime.Today;
			var idade = lData.Year - pDateTime.Year;
		...
		{
			return pDateTime >= SqlDateTime.MinValue.Value
				   && pDateTime <= SqlDateTime.MaxValue.Value;
		}
		...
		/// <returns>
		///     Diferença em dias entre a data inicial e final
		///     positivo se data inicial menor que data final
		///     negativo se data inicial maior que data final
		/// </returns>
		public static int TotalDays(this DateTime lData1, DateTime lData2, EModeDiffDate pMode,
			params DateTime[] pFeriados)
		{
			...
			switch (pMode)
			{
				case EModeDiffDate.AllDays:
					result = (int)(pData2 - pData1).TotalDays;
					break;
				case EModeDiffDate.WorkDays:
					{
						result = (int)(pData2 - pData1).TotalDays;
						var aux = pData2.AddDays(-result % 7);
						result -= result / 7 * 2;
						while (aux <= pData2)
						{
						...
						if (pFeriados != null)
							result -= pFeriados.Where(d =>
								!d.DayOfWeek.In(DayOfWeek.Saturday, DayOfWeek.Sunday) &&
								d.Between(pData1.AddDays(1), pData2)).Count();
						break;
					}
				case EModeDiffDate.WorkMonth:
				...
				case (EModeDiffDate)4:
					{
						for (result = 0; pData1 < pData2; result++)
						{
							...
							if (pData1.DayOfWeek.In(DayOfWeek.Saturday, DayOfWeek.Sunday) ||
								pFeriados != null && pFeriados.Contains(pData1))
								result--;
						}
		...
		{
			var result = pData.AddDays(days);
			if (days != 0)
			{
				var inc = days / Math.Abs(days);
				while (pData.TotalDays(result, EModeDiffDate.WorkDays, feriados) < days)
		...
		{
			if (pDataRef == null) throw new ArgumentNullException("pDataRef");
			return new DateTime(pDataRef.Year, 1, 1).AddDays(-1)
				.TotalDays(new DateTime(pDataRef.Year, 12, 31), pMode, pFeriados);
		}
		...
		{
			return !pDateToTest.DayOfWeek.In(DayOfWeek.Saturday, DayOfWeek.Sunday) &&
				   !pHolidays.Contains(pDateToTest.Date);
		}
		...
		{
			for (var totalDias = 0; totalDias < pDaysToAdd; totalDias++)
				for (; ; )
				{
		...
		/// <param name="pHolidays"></param>
		/// <returns></returns>
		public static DateTime PreviousWorkDay(this DateTime pDateToDecrease, int pDaysToDecrease = 1,
			params DateTime[] pHolidays)
		{
			for (var totalDias = 0; totalDias < pDaysToDecrease; totalDias++)
				for (; ; )
				{
		...
		public static int CountWorkDays(this DateTime pStart, DateTime pEnd, params DateTime[] pHolidays)
		{
			...
			var totFeriados = pHolidays.Count(d =>
				!d.DayOfWeek.In(DayOfWeek.Saturday, DayOfWeek.Sunday) && d >= pStart && d <= pEnd);
			// total de dias corridos entre as datas.
			var totDiasCorridos = (pEnd - pStart).Days;
			...
			var totSemanas = (int)Math.Floor((double)(totDiasCorridos / 7)) +
							 (pStart.DayOfWeek > pEnd.DayOfWeek ? 1 : 0);
			// fórmula final
			return totDiasCorridos - totFeriados - totSemanas * 2 -
				   (pEnd.DayOfWeek == DayOfWeek.Saturday ? 1 : 0); // sábado é o fim da semana de 7 dias.
		}
		...
		{
			var dia = ((decimal)pData.Day).GetExtensoFmt("dia", "dias", "", "");
		...
		{
			return Enumerable.Range(0, (pEnd.Date - pStart.Date).TotalDays.ToInt32() + 1)
				.Select(x => pStart.AddDays(x));
		}
			...
			Expression<Func<T, bool>> f2)
		{
			...
			var lambdaExpression = RebuildExpressionIfNeeded(f1.Parameters[0], f2);
		...
		public static Expression<Func<TResult, bool>> Cast<T, TResult>(this Expression<Func<T, bool>> f1)
			where TResult : T
		{
		...
		/// <returns></returns>
		private static LambdaExpression RebuildExpressionIfNeeded<T>(ParameterExpression p,
			Expression<Func<T, bool>> expr2)
		{
			LambdaExpression lambdaExpression;
			var expression = new ParameterModifier().Modify(expr2, p);
		...
		public static string GetPropertyName<TSource, TResult>(this TSource pObject,
			Expression<Func<TSource, TResult>> pExpression)
		{
			var lUnaryExpression = pExpression.Body as UnaryExpression;
			var lMemberExpression = pExpression.Body as MemberExpression;
			...
			var f = (Expression<Func<TSource, object>>)Expression.Lambda(
				typeof(Func<TSource, object>),
				Expression.Convert(lMemberExpression.Expression, typeof(object)),
				pExpression.Parameters.ToArray());
			var p = GetPropertyName(pObject, f);
			if (p != "") return p + "." + lMemberExpression.Member.Name;
			return lMemberExpression.Member.Name;
		}

		public static PropertyInfo GetProperty<TSource, TResult>(this Type pObject,
			Expression<Func<TSource, TResult>> pExpression)
		{
			var lUnaryExpression = pExpression.Body as UnaryExpression;
			var lMemberExpression = pExpression.Body as MemberExpression;
			...
			if (lMemberExpression == null)
				throw new InvalidOperationException("pExpression must be a member expression");
		...
		/// <returns>
		///     Se o objeto não for nulo, o valor da propriedade acessada será retornado normalmente. Se o objeto for nulo,
		///     será retornado o valor Default da propriedade que se deseja acessar.
		/// </returns>
		...
		/// <returns>
		///     Se o objeto não for nulo, o valor da propriedade acessada será retornado normalmente. Se o objeto for nulo,
		///     será retornado o valor passado por parâmetro.
		/// </returns>
		...
		public static int ToInt32(this Enum pEnum)
		{
		...
		public static string GetFriendlyName(this Enum pEnum)
		{
			...
			if (pEnum.IsDefined())
			{
				var lFI = pEnum.GetType().GetField(pEnum.ToString());
				var lAtt = (FriendlyNameAttribute)lFI.GetCustomAttributes(typeof(FriendlyNameAttribute), false)
					.FirstOrDefault();
				return lAtt != null ? lAtt.NameSingle : pEnum.ToString().i18n();
			}

			if (pEnum.GetType().GetCustomAttributes(typeof(FlagsAttribute), false) != null && pEnum.ToInt32() != 0)
				return pEnum.ToString().Split(',')
					.Select(x => (Enum.Parse(pEnum.GetType(), x) as Enum).GetFriendlyName()).StringJoin(", ");
			return "";
		}
		...
		public static Dictionary<int, string> GetValuesAndDescriptions(this Type pEnum)
		{
			var dic = new Dictionary<int, string>();
			if (pEnum.IsEnum)
			{
				var values = Enum.GetValues(pEnum);

				foreach (int val in values)
				{
					var memInfo = pEnum.GetMember(pEnum.GetEnumName(val));
					var descriptionAttribute = memInfo[0]
						.GetCustomAttributes(typeof(DescriptionAttribute), false)
						.FirstOrDefault() as DescriptionAttribute;

					if (descriptionAttribute != null) dic.Add(val, descriptionAttribute.Description);
				}
			}
		...
		public static T? As<T>(this DataRow pDataRow, int columnIndex)
			where T : struct
		{
			var result = pDataRow[columnIndex];
		...
		public static T? As<T>(this DataRow pDataRow, string columnName)
			where T : struct
		{
			var result = pDataRow[columnName];
			if (result is DBNull) return null;
			if (result is T) return (T)result;
			return (T)Convert.ChangeType(result, typeof(T));
		}
